...
const char *ART_HOOK_TAG = "art_hook";
...
bool verifyMethodSize(JNIEnv *env, jobject method1, jobject method2) {
    ...
    logd(ART_HOOK_TAG, "ArtMethod1 addr=%p\n", artMethod1);
    logd(ART_HOOK_TAG, "ArtMethod2 addr=%p\n", artMethod2);
    ...
    logd(ART_HOOK_TAG, "ArtMethod compile size=%zu\n", artMethodCompileSize);
    logd(ART_HOOK_TAG, "ArtMethod runtime size=%zu\n", artMethodRuntimeSize);
    ...
    for (int i = 0; i < artMethodRuntimeSize; i++) {
        logd(ART_HOOK_TAG, "%02X", copiedArtMethod[i]);
    }
    art::mirror::ArtMethod *artMethod = static_cast<art::mirror::ArtMethod *>(artMethod1);
    logd(ART_HOOK_TAG, "ptr_sized_fields_.data_=%p", artMethod->ptr_sized_fields_.data_);
    logd(ART_HOOK_TAG, "ptr_sized_fields_.quick=%p",
...
printRegister(uint64_t p0, uint64_t p1, uint64_t p2, uint64_t p3, uint64_t p4, uint64_t p5,
    ...
    logd(ART_HOOK_TAG, "reg: [%02lX,%02lX,%02lX,%02lX,%02lX,%02lX,%02lX,%02lX]", p0, p1, p2, p3, p4,
         p5, p6, p7);
}
...
extern "C" void parkHookBefore() {
    ...
    logd(ART_HOOK_TAG, "parkHookBefore()");
}

extern "C" void parkHookAfter() {
    logd(ART_HOOK_TAG, "parkHookAfter()");
...
bool hookArtMethod(JNIEnv *env, jobject method) {
    ...
    logd(ART_HOOK_TAG, "origin quick=%p", backup_method_ptr);
    ...
    logd(ART_HOOK_TAG, "hook addr=%p, shell code=%p", hook_stub_method_ptr, shell_code_func);
    inHookArtMethod->ptr_sized_fields_.entry_point_from_quick_compiled_code_ = hook_stub_method_ptr;
    return JNI_TRUE;
}
